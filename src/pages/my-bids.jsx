              ...
              {allBiders && allBiders.length > 0 ? (
                allBiders.map((bid, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition duration-200">
                    ...
                    <td className="px-6 py-4 text-gray-700">{bid?.buyerDetails?.name}</td>
                    <td className="px-6 py-4 text-gray-700">
                      {bid?.vehicleDetails?.make}/{bid?.vehicleDetails?.model}/{bid?.vehicleDetails?.series}
                    </td>
                    <td className="px-6 py-4 font-semibold text-[#191970]">PKR {bid?.bidDetails?.yourOffer || "0"}</td>
                    <td className="px-6 py-4 text-gray-500">{bid?.bidDetails?.bidCreatedAt?.slice(0, 10)}</td>
                ...
                <tr>
                  <td colSpan="6" className="text-center py-10 text-gray-400 font-medium">No bids yet.</td>
                </tr>
              )}
            </tbody>
          </table>
              ...
              <div key={index} className="bg-white rounded-xl shadow-md border border-gray-200 p-4">
                ...
                <div className="space-y-2 text-sm">
                  <p className="flex justify-between">
                    ...
                    <span className="text-gray-800 cursor-pointer underline">{bid?.buyerDetails?.name}</span>
                  </p>
                  <p className="flex justify-between">
                  ...
                  <p className="flex justify-between">
                    ...
                    <span className="text-[#191970] font-semibold">PKR {bid?.bidDetails.yourOffer || "0"}</span>
                  </p>
                  <p className="flex justify-between">
                    <span className="text-gray-900 font-bold">Date</span>
                    <span className="text-gray-500">{bid?.bidDetails?.bidCreatedAt?.slice(0, 10)}</span>
                  </p>
                </div>
              </div>
            ))
