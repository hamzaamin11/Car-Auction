...
export const EditIncrement = ({
  handleClose,
  handleGetAllCommission,
  incrementId,
}) => {
  const [UpdateIncrementAmount, setUpdateIncrementAmount] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(
        `${BASE_URL}/admin/updateConfigureIncrement/${incrementId}`,
        {
          amount: UpdateIncrementAmount,
        }
      );
      console.log();
      handleClose();
      ...
      await Swal.fire({
        title: "Success!",
        text: "The Increment has been updated successfully.",
        icon: "success",
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 border ">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              value={UpdateIncrementAmount}
              onChange={(e) => {
                const value = e.target.value;

                // Allow only numbers + up to 3 decimals
                if (/^\d*\.?\d{0,3}$/.test(value)) {
                  setUpdateIncrementAmount(value);
                }
              }}
