...
export default function CustomerList() {
  ...
  const handleGetCustomersNotification = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/customer/getImportedCar`);
  ...
  const handleDeleteNotification = async (id) => {
    try {
      const res = await axios.patch(
        `${BASE_URL}/customer/deleteImportedCar/${id}`
      );
      console.log(res.data);
              ...
              {allNotification?.map((user, idx) => (
                <tr key={user.id} className={idx % 2 === 0 ? "bg-gray-50" : ""}>
                  ...
                  <td className="px-4 py-3 text-center space-x-2">
                    <button
                      onClick={() => handleDeleteNotification(user.id)}
